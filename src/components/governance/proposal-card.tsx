...
export function ProposalCard({ proposal }: { proposal: Proposal }) {
  ...
  const handleVote = (direction: "for" | "against") => {
    ...
    toast.success(
      ...
      `Your ${direction} vote for proposal #${proposal.id} has been captured.`,
    );
  };
      ...
      <div className="flex items-start justify-between mb-2">
        ...
        {isActive && (
          <div className="flex items-center gap-1 text-xs text-gray-400">
            ...
            <span>
              Ends{" "}
              {formatDistanceToNow(new Date(proposal.deadline), {
                addSuffix: true,
              })}
            </span>
          </div>
      ...
      <div className="mb-3">
        <div className="flex justify-between text-xs text-gray-500 mb-1">
          ...
          <span className="text-red-500 font-medium">
            ✗ {proposal.votesAgainst} Against ({100 - forPct}%)
          </span>
        </div>
        ...
        <p className="text-xs text-gray-400 mt-1">
          ...
          {total >= proposal.quorum && (
            <span className="text-green-600 ml-1">✓ Reached</span>
          )}
        </p>
      </div>
